...
def init_defense_model(class_name=None):
    ...
    if len(class_name) > 20:
        ...
        recon_model_path = '/opt/data/private/Attack/PCBA/checkpoint/2024/rec-shape-dim-512/best_parameters.tar'
    print("recon_model_path==>>",recon_model_path)
...
def eval_ASR(model, test_loader, args, val_attack, class_name):
    ...
    pdb.set_trace()
    # defense_data_path = '/opt/data/private/Attack/HiT-ADV/defense_data'
    # ***** EVALUATING ***** ***** ***** ***** ***** ***** ***** *****
    pbar = tqdm(test_loader) # total size 10 bs-256
    all_size = 0 
    for i, (ori_data, label) in enumerate(pbar):
        all_size += ori_data.shape[0]
        batch += 1
        ...
        ### load defense
        # load_data_path = os.path.join(defense_data_path, args.model)
        ###
        ...
        with torch.no_grad():
            ...
            ## 
            # ori_data = defense_func(ori_data)
            ori_data = ori_data.transpose(2,1)
            pred_clip, text_feat = clip_model(ori_data, label)
            ntargets = pred_clip.data.max(1)[1]
            ori_data = ori_data.transpose(2,1)
            _, _, _, ori_data = rec_model(ori_data, ntargets)
            ...
            adv_data_label.append(adv_preds)
            origin_data_label.append(ori_preds)

            ## defense results
            adv_data = adv_data.transpose(2,1)
            pred_clip, text_feat = clip_model(adv_data, label)
            ntargets = pred_clip.data.max(1)[1]
            ntargets_mask = (ntargets == label)
            # print("label:",label)
            # print("ntargets:",ntargets)
            adv_total_mask = mask_ori & (~mask_adv) ## 被攻击的总数量
            adv_total_size += adv_total_mask.sum().float().item()

            adv_data = adv_data.transpose(2,1)
            _, _, _, rec_adv_data = rec_model(adv_data, ntargets)
            if args.model.lower() == 'pointnet':
                rec_adv_logits, _ = model(rec_adv_data)
            else:
                rec_adv_logits = model(rec_adv_data)

            defense_pred = torch.argmax(rec_adv_logits, dim=-1)
            defense_mask = (defense_pred == label)
            success_defense_size = (defense_mask * adv_total_mask).sum().float().item()
            defense_total_size += success_defense_size

            defense_mask = ntargets_mask
            success_defense_size = (defense_mask * adv_total_mask).sum().float().item()
            cdefense_total_size += success_defense_size

            true_total_size += mask_ori.sum().item()
            ori_total_size += (defense_mask * mask_ori).sum().item()

    adv_data_list = torch.cat(adv_data_list, dim=0)
    adv_data_label = torch.cat(adv_data_label, dim=0)
    origin_data_label = torch.cat(origin_data_label, dim=0)

    adv_data_list = adv_data_list.transpose(1,2).detach().cpu().numpy()
    adv_data_label = adv_data_label.detach().cpu().numpy()
    origin_data_label = origin_data_label.detach().cpu().numpy()

    ### save defense
    # defense_data_path = '/opt/data/private/Attack/HiT-ADV/defense_data'
    # defense_save_dir = os.path.join(defense_data_path, args.model)
    # if not os.path.exists(defense_save_dir):
    #     os.mkdir(defense_save_dir)
    
    # np.save(os.path.join(defense_save_dir,'model40_adv.npy'),adv_data_list)
    # np.save(os.path.join(defense_save_dir,'model40_adv_label.npy'),adv_data_label)
    # np.save(os.path.join(defense_save_dir,'model40_ori_label.npy'),origin_data_label)
    ...
    print("clean acc ===>>",float(true_total_size/all_size))
    logger.info(f'Overall CLEAN ACC: {float(true_total_size/all_size)}')
    # ***** END LOGGING ***** ***** ***** ***** ***** ***** ***** *****
    print(defense_total_size,"<--->",adv_total_size)
    print(" origin label pred ==>>",float(ori_total_size/true_total_size))
